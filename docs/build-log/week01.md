...
Local Authoring: Write weekly updates in standard .md files.
Version Control: Run git push to send the new logs to the main branch.
Automated Build: A GitHub Action detects the push, spins up an Ubuntu runner, installs Python and MkDocs, and compiles the Markdown into a static HTML/CSS website.
Deployment: The Action pushes the compiled site to a hidden gh-pages branch, which GitHub Pages then serves to the live URL.
Code Implementation
Two core configuration files drive this setup.

1. Site Configuration (mkdocs.yml) This dictates the site's structure, theme, and features, including syntax highlighting for future C/MicroPython code:
...
2. Deployment Pipeline (.github/workflows/publish.yml) This YAML script handles the automation, ensuring the MkDocs build process runs on GitHub's servers every time a commit is pushed to the main branch.
